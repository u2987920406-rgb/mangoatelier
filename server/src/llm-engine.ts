...
//   - 'openai'  → endpoint compatible OpenAI (DeepSeek, etc.) — futur cloud
//
 ...
 * 3 valeurs valides. `envValue` = la variable dédiée d'une feature. */
export function resolveProvider(envValue?: string, fallback: LLMProvider = 'claude'): LLMProvider {
  ...
  return raw === 'ollama' || raw === 'claude' || raw === 'openai' ? raw : fallback
}
...
async function askOpenAI(
  ...
  const base = (process.env.LLM_OPENAI_URL ?? process.env.ELEVE_API_URL ?? 'https://api.deepseek.com/v1')
    .trim()
    .replace(/\/+$/, '')
  const url = base.endsWith('/chat/completions') ? base : `${base}/chat/completions`
  const key = (process.env.LLM_OPENAI_KEY ?? process.env.ELEVE_API_KEY ?? '').trim()
