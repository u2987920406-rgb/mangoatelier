...
const TUTORIAL_1: TutorialStep[] = [
  ...
  step("t1-write-idea", "Écris une idée", "Essaie : clique dans la zone de texte et écris une idée d'app, même vague. Tu n'as encore rien à lancer — on découvre l'interface.", "type", 0, { target: "textarea" }),
  ...
  step("t1-modes", "Mode MVP / Élite / Finition", "Trois régimes : ⚡ MVP (rapide, économe), 💎 Élite (cadrage + tout l'arsenal), 🛡️ Finition (durcissement après 80%). Tu choisis selon l'ambition du moment.", "observe", 0, { target: ".mode-dropdown" }),
  step("t1-model", "Le choix du cerveau", "Tu peux changer le moteur : Haiku (rapide), Sonnet (équilibré), Opus (qualité max), ou l'Élève local ($0). Chaque tâche son cerveau.", "observe", 0, { target: ".model-dropdown" }),
  step("t1-preview", "L'aperçu live", "À droite, ton app tourne pour de vrai et se recharge à chaque modification. C'est ton retour visuel immédiat.", "observe", 0, { target: ".preview-iframe" }),
  step("t1-memory", "La mémoire", "MangoAI retient ton style, ton vocabulaire et tes préférences entre les projets. Ce panneau montre ce qu'il a appris de toi.", "observe", 0, { target: "[data-panel=knowledge]" }),
  step("t1-versions", "Les versions & le rollback", "Chaque tour est versionné. Tu peux comparer et revenir en arrière en un clic — itère sans peur de casser.", "observe", 0, { target: "[data-panel=versions]" }),
  step("t1-snap", "Le Snap", "Tu pourras dessiner une zone sur l'aperçu pour cibler exactement ce qu'il faut corriger. Pratique pour un détail visuel précis.", "observe", 0, { target: "[data-snap]" }),
  step("t1-inspect", "Le clic → source", "En mode inspection, cliquer un élément de l'aperçu arme une édition chirurgicale du bon fichier:ligne. Du pixel au code, directement.", "observe", 0, { target: "[data-inspect]" }),
...
const TUTORIAL_2: TutorialStep[] = [
  ...
  step("t2-name", "Nomme le projet", "Donne un nom à ton projet — par exemple « landing-cafe ». Le nom devient le dossier de travail.", "type", 10, { target: "input[placeholder='nom-du-projet']" }),
  ...
  step("t2-watch", "Regarde construire", "MangoAI réfléchit puis écrit le code. L'aperçu se met à jour tout seul. Laisse-le finir le tour.", "observe", 10, { target: ".preview-iframe" }),
  ...
  step("t2-snap", "Cible une zone", "Active le Snap et entoure une zone à corriger (ex. le menu). MangoAI saura exactement quoi retoucher.", "observe", 10, { target: "[data-snap]" }),
  step("t2-rollback", "Reviens en arrière", "Ouvre les versions et reviens à un état précédent en un clic. Itérer sans risque, c'est le cœur du flux.", "observe", 10, { target: "[data-panel=versions]" }),
