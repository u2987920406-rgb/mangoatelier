...
**🎉 Infrastructure du Kernel COMPLÈTE (5/5 piliers).** Reste = **INTÉGRATION** (phase suivante) : (1) brancher MangoQA 3 visages sur le Bus via l'observateur `*` (disjoncteur déterministe + observateur-conseil qui lit `kernel.trace` + œil design souple) ; (2) persistance/SQLite-vec du Blackboard quand le besoin arrivera ; (3) migrer progressivement `agent.ts` / le chat sur `getBrain()` + `getBus()` + `getToolRegistry()` + `getTracer()` (aujourd'hui `query()`/`askLLM`/`visionServer` directs) — migration non bloquante, le chat existant reste intact entre-temps.
