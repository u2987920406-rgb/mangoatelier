...
| 56 | **Système Tutorial Orchestral** — 10 tutoriels progressifs (liberté **0 → 100 %**), **double apprentissage** (Mango apprend Raf ↔ Raf apprend Mango), chantiers A→D, `TutorialSpotlight`/`Tutorial`/`RelationshipCard`, axiomes tagués `[tutoriel-N]`. **Chantier principal approuvé** — plan complet dans `TUTORIAL-PLAN.md`. Cold start de toute la machine d'apprentissage | 📐 approuvé · Chantier A à coder | 🧠 Opus 4.8 | XL |
| 57 | **Indépendance fournisseur LLM** — une seule var `.env` (`LLM_PROVIDER`) pilote **TOUT** (génération principale + Hermes + feedback + tags), providers nommés deepseek/mistral/groq/openai/ollama. Routeur `llm-engine.ts` déjà à **~80 %** (askLLM + resolveProvider OK ; reste : brancher `agent.ts`/`review.ts` + alias providers). DeepSeek V3 ≈ qualité Sonnet à 10× moins cher | 🚧 ~80 % · **prioritaire** (2-3h) | ⚖️ Sonnet 4.6 | S |
