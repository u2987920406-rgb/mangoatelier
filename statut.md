...
| **D — Branchement de l'Élève + boucle de relais** | Modèle OSS local en mode contrat ; **tests auto (idée 24)** + inspection objective (compile/runtime/tests/compteur de boucles) qui décide d'escalader ; relais auto vers Claude ; **validation de chaque axiome contre l'Élève** ; courbe du taux d'intervention → 0 % sur la routine (greffée au panneau du Jalon B) | 💤 quand un modèle local candidat existe |
