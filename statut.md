...
- **Phase 3 — Audit Scan** (`server/src/audit-scan.ts`, CLI **jamais importé en prod**) : suite **figée held-out** (`audit-tasks.ts`, jamais source d'axiome), score 100 % objectif (build/1er tour/tentatives/temps) de l'Élève **seul**, empreintes de repro (digest modèle + version Ollama + hash axiomes), comparaison au scan précédent, et **mode `--ablate`** (suite AVEC vs SANS le dernier axiome → attribution causale des « mauvais plis », restauration byte-identique du registre). Prouvé : scan 5/5 build / 100 % 1er tour ; ablation NEUTRE + registre intact. Journal `.audit.jsonl` (workspace/, git-ignoré).
...
3. **`selectAxioms` v2.1 — détecter le type de projet depuis la mémoire projet**, pas seulement les mots-clés du prompt (plus robuste pour la récupération par type).
...
- **Le SEUL vrai point dur, ce n'est PAS la vision — c'est pixel → code.** Et SAM ne le résout pas (un masque dit *où à l'écran*, jamais *quelle ligne de TSX*). Mais on **contrôle le DOM et le build**, donc on contourne la vision : `document.elementFromPoint(x,y)` (Playwright) → le nœud DOM EXACT (plus précis qu'un masque, déterministe, gratuit) ; en dev, la fiber React expose `_debugSource` = `{ fileName, lineNumber }` → le crochet vers le code, offert par le JSX dev transform de Vite.
- **Architecture simplifiée** : clic (x,y) → `elementFromPoint` + `_debugSource` + crop de l'élément (tool snapshot existant) → **Claude** (déjà le VLM de la boucle) fait l'edit via la **Coque Rigide**. → ❌ SAM (remplacé par `elementFromPoint`), ❌ VLM séparé (c'est Claude), ✅ Coque Rigide (exécuteur). **Une seule brique neuve** : un relais clic→`{source, crop, code}`. C'est la feature **« Snap » existante + un mapping source + le contrat**.
...
- **Reco de phasage** : (1) piste future n°1 = fournir le code du fichier cible à l'Élève [débloque les edits, prérequis dur] ; (2) le relais clic→source (`elementFromPoint` + `_debugSource`) ; (3) Claude édite via le contrat ; (4) plus tard, déléguer à un Qwen-VL local pour le coût zéro. **Priorité haute à terme ; faisable sans dépendance externe.**
