...
| 56 | **Système Tutorial Orchestral** — 10 tutoriels progressifs (liberté **0 → 100 %**), **double apprentissage** (Mango apprend Raf ↔ Raf apprend Mango), chantiers A→D. Plan complet dans `TUTORIAL-PLAN.md`. **Chantiers A+B+C ✅** : A = squelette + persistance + tutos 1-2 ; B = spotlight (trou+beacon+gradient de liberté) + RelationshipCard + enchaînement ; **C = moteur d'apprentissage** : `tutorial-feedback.ts` (retour checkpoint 👍/👎 → axiome tagué `[tutoriel-N]` via askLLM) + routes feedback/relationship, bloc `tutorial` en tête des 3 scénarios (posture pédagogue, threadé via `/api/chat`→`runAgent`), RelationshipCard affiche les vrais axiomes appris. Tests 28/28 + 13/13 + gating. Reste D (tutos 3-10) | 🚧 Chantiers A+B+C ✅ · D à venir | 🧠 Opus 4.8 | XL |
