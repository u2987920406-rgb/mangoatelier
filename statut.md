...
| 56 | **Système Tutorial Orchestral** — 10 tutoriels progressifs (liberté **0 → 100 %**), **double apprentissage** (Mango apprend Raf ↔ Raf apprend Mango), chantiers A→D. Plan complet dans `TUTORIAL-PLAN.md`. **Chantier A ✅ livré** (`tutorial.ts` + routes `/api/tutorial*` + `Tutorial.jsx` overlay + App/Home + tutos 1-2 définis, test 28/28). Restent B (spotlight + RelationshipCard + verrous liberté), C (feedback→axiomes + bloc `scenario.ts` + mini-review), D (tutos 3-10) | 🚧 Chantier A ✅ · B/C/D à venir | 🧠 Opus 4.8 | XL |
