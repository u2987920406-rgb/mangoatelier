...
- [x] **Idée 12 · Sélecteur ⚡ MVP / 💎 Élite** 🔧 *Type A — FAIT (2026-06-13)*
  - Bouton dans le header (comme le choix de modèle) ✅
  - Mode MVP = flux rapide actuel (thinking off, rituel off, budget vision 3) ; Mode Élite = tout l'arsenal ✅
  - Prérequis de toutes les fonctions avancées futures — l'interrupteur est en place
  - **Statut :** ✅ FAIT — testé e2e sur les deux modes

- [x] **Idée 16 · Intégration GitHub native** 🔧 *Type A — FAIT (2026-06-13, push à valider)*
  - Push vers un repo GitHub en 1 clic depuis MangoAI ✅ (code livré)
  - Bouton "GitHub"/"Push" + chip vers le repo dans le header
  - **Action requise de ta part** : créer un token GitHub (scope `repo`) sur
    https://github.com/settings/tokens, le mettre dans `server/.env`
    (`GITHUB_TOKEN=ghp_...`), relancer le serveur → le bouton GitHub apparaît.
    Puis cliquer "GitHub" sur un projet pour valider le push réel.
  - **Statut :** ✅ code fait, ⚠ push réel à tester avec ton token

- [x] **Idée 17 · Intégration Supabase** 🔧 *Type A — FAIT (2026-06-13, à tester)*
  - L'agent sait générer le code Supabase (client, auth, SQL + RLS) ✅
  - Ouvre la génération d'apps avec login + base de données
  - Sécurité : `.env` git-ignoré + hors zip (clés protégées)
  - **Action requise de ta part** : créer un projet gratuit sur supabase.com,
    mettre `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` dans le `.env` du
    projet généré, puis demander à MangoAI une app avec données/login pour valider.
  - **Statut :** ✅ code fait, ⚠ à tester avec tes clés Supabase
...
- [x] **Idée 10 · Knowledge Flywheel** 🔧 *Type A — FAIT (2026-06-13), testé e2e*
  - `review.ts` cure un 4ᵉ magasin `.axioms.md` (axiomes universels) ✅
  - Format `AXIOME-[CAT]-[NN]` + maturité candidat→confirmé + daté ✅
  - Garde-fous codés : falsifiable, plafond ~12/3000 car., demande user prime ✅
  - Testé : `AXIOME-UIUX-01` extrait d'un vrai tour, visible dans le panneau Mémoire
  - **Statut :** ✅ FAIT

- [x] **Idées 8 + 6 · Blueprints + Stacks par type de projet** 🔧 *Type A — FAIT (2026-06-13)*
  - Catalogue `blueprints.ts` : 6 types (vitrine, web app, dashboard, jeu 2D, slides, agent IA) ✅
  - Stack optimale + arborescence type par projet, profondeur « structurée mais souple »
  - Testé : l'agent restitue fidèlement stack+arbo par type
  - **Statut :** ✅ FAIT

- [x] **Idées 9 + 11 · Mango Plan + Moodboard automatisé** 🔧 *Type A — FAIT (2026-06-13, Élite)*
  - Mango Plan : ≤3 questions → plan.md → validation → code (Élite) ✅
  - Moodboard v1 : WebSearch/WebFetch des leaders → règles de design dans plan.md ✅
  - Zéro dette technique : concevoir avant de coder ✅
  - *Reporté v2* : capture visuelle autonome des leaders (Playwright externe)
  - **Statut :** ✅ FAIT (v1) — Phase 2 complète
...
**🎉 Phase 1 terminée côté code (idées 12, 16, 17).** Deux choses t'attendent pour valider :
1. **GitHub** : crée un token (scope `repo`) → `GITHUB_TOKEN` dans `server/.env` → clique « GitHub » sur un projet.
2. **Supabase** : crée un projet gratuit sur supabase.com → mets les clés dans le `.env` du projet → demande une app avec données/login.

**🎉 Phases 1 ET 2 complètes côté code.** MangoAI a son interrupteur (12), GitHub (16), Supabase (17), son flywheel d'axiomes (10), ses blueprints par type (6+8) et son mode architecte Mango Plan + moodboard (9+11).

**→ Prochaine étape : au choix**
- **Phase 4** (vision avancée : idées 7, 1, 2) ou **Phase 5** (écosystème : Figma 25, déploiement 18, tests 24, métriques 21).
- **Le plan plus ambitieux** que tu as en tête (à définir ensemble).
- Validations en attente : GitHub (`GITHUB_TOKEN`), Supabase (clés), et tu peux voir Mango Plan/blueprints en action dans un vrai projet.

*(La Phase 3 — formation — est mise de côté à ta demande.)*
