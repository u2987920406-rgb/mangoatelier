...
- **Plan de formation & développement** : `PLAN-FORMATION.md` — 26 idées, 6 phases, timing, checkboxes, guides pour les projets formation. À relire au démarrage de session et à tenir à jour. **Phase 1 FAITE côté code : idées 12 (MVP/Élite) ✅, 16 (GitHub) ✅, 17 (Supabase) ✅. Prochaine étape : Phase 2, idée 10 (Knowledge Flywheel). ⚠ 2 validations en attente de l'utilisateur : (a) push GitHub réel → `GITHUB_TOKEN` scope `repo` dans `server/.env` ; (b) app Supabase → clés `VITE_SUPABASE_*` dans le `.env` du projet généré**
